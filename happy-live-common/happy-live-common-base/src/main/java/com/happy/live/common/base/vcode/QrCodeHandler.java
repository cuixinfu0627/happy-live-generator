...
public class QrCodeHandler {
    ...
    public static String encode(String content, int width, int height,
        ...
        try {
            ...
            if(width<260||height<260){
                width = QrCodeHandler.WIDTH;
                height = QrCodeHandler.HEIGHT;
            }
            ImageIO.write(genBarcode(content, width, height, iconImagePath,hasFiller),
                    "png", new File(qrcodeImagePath));
            log.info("二维码已生成  "+qrcodeImagePath);
            return qrcodeImagePath;
        } catch (IOException e) {
        	log.error("图片读取异常 ： "+ StringHandler.getExceptionStack(e));
        } catch (WriterException e) {
        	log.error("图片输出异常 ："+StringHandler.getExceptionStack(e));
        }
        return null;
    }
	...
	public static void saveImage(BufferedImage bi ){ 
        try { 
            ...
            ImageIO.write(bi,"jpg",new File("E:/Test/qrcode/gray.jpg")); 
        } catch (IOException e) { 
        	log.error("图片写入异常 ："+StringHandler.getExceptionStack(e));
            e.printStackTrace(); 
        } 
    }
	
    public static void main(String[] args) {
        String codeIconPath = "E:/Test/logo.jpg";
        String codePath = "E:/Test/qrcode/"+new Date().getTime()+".png";
        QrCodeHandler.encode("http://39.106.75.108",300, 300, codeIconPath, codePath,true);
        System.out.println(" 生成完毕" + codePath);
    }
}